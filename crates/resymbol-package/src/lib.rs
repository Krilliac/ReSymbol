...
mod tests {
    ...
    fn deterministic_round_trip_canonicalizes_nested_keys() {
        ...
        assert_eq!(
            ...
            concat!(
                ...
                "\"values\":[1,2,3]},\"schema_version\":15}"
            )
        );
    ...
    fn unsupported_schema_is_reported_before_payload_decoding() {
        let bytes = format!(
            "{{\"schema_version\":16,\"generator_version\":\"0.1.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":\"not the requested type\"}}"
        );
    ...
    fn unknown_envelope_fields_are_rejected() {
        let bytes = format!(
            "{{\"schema_version\":14,\"generator_version\":\"0.1.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":null,\"unexpected\":true}}"
        );
    ...
    fn explicit_compatibility_range_can_accept_a_newer_envelope() {
        let bytes = format!(
            "{{\"schema_version\":15,\"generator_version\":\"0.2.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let compatibility = SchemaCompatibility::inclusive(14, 15).expect("valid range");
    ...
    #[test]
    fn schema_v3_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":3,\"generator_version\":\"0.1.0-alpha.3\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 3 is accepted explicitly");
    ...
    #[test]
    fn schema_v4_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":4,\"generator_version\":\"0.1.0-alpha.4\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 4 is accepted explicitly");
    ...
    #[test]
    fn schema_v5_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":5,\"generator_version\":\"0.1.0-alpha.5\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 5 is accepted explicitly");
    ...
    #[test]
    fn schema_v6_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":6,\"generator_version\":\"0.1.0-alpha.6\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 6 is accepted explicitly");
    ...
    #[test]
    fn schema_v7_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":7,\"generator_version\":\"0.1.0-alpha.7\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 7 is accepted explicitly");
    ...
    #[test]
    fn schema_v8_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":8,\"generator_version\":\"0.1.0-alpha.8\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 8 is accepted explicitly");
    ...
    #[test]
    fn schema_v9_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":9,\"generator_version\":\"0.1.0-alpha.9\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 9 is accepted explicitly");
    ...
    #[test]
    fn schema_v10_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":10,\"generator_version\":\"0.1.0-alpha.10\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 10 is accepted explicitly");
    ...
    #[test]
    fn schema_v11_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":11,\"generator_version\":\"0.1.0-alpha.11\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 11 is accepted explicitly");
    ...
    #[test]
    fn schema_v12_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":12,\"generator_version\":\"0.1.0-alpha.12\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 12 is accepted explicitly");
    ...
    #[test]
    fn schema_v13_requires_explicit_read_compatibility_and_remains_decodable() {
        let bytes = format!(
            "{{\"schema_version\":13,\"generator_version\":\"0.1.0-alpha.13\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let error = from_slice::<u64>(bytes.as_bytes())
            .expect_err("the default policy accepts only newly emitted packages");
        assert!(matches!(
        ...
        let options = PackageOptions::new(1024, compatibility).expect("valid options");
        let decoded = from_slice_with_options::<u64>(bytes.as_bytes(), options)
            .expect("schema 13 is accepted explicitly");

        assert_eq!(decoded.schema_version(), 13);
        assert_eq!(*decoded.payload(), 42);
    }
    ...
    fn bound_decode_rejects_envelope_payload_identity_mismatch() {
        let bytes = format!(
            "{{\"schema_version\":15,\"generator_version\":\"0.1.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":{{\"binary_id\":\"{DIGEST_B}\",\"value\":7}}}}"
        );
        let error =
