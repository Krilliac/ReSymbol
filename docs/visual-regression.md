...
ReSymbol's standard deterministic capture contract covers eleven scenarios for the checked-in
...
`resymbol-workbench --features screenshot --bin resymbol-workbench`. A default run writes the eleven
...
thresholds. The current manifest identifies the five legacy reference PNGs explicitly; the next
intentional default refresh promotes the complete eleven-scenario standard set without pretending
that uncaptured pixels have already been reviewed.

When intentionally updating the references, run the script with its default output directory,
inspect all eleven full-size images for layout, clipping, stale or duplicate widget warnings,
...
Rust dependency changes. It regenerates all eleven standard scenarios plus a separate focused
