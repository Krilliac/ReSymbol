...
mod tests {
    ...
    fn unsupported_schema_is_reported_before_payload_decoding() {
        let bytes = format!(
            "{{\"schema_version\":7,\"generator_version\":\"0.1.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":\"not the requested type\"}}"
        );
    ...
    fn unknown_envelope_fields_are_rejected() {
        let bytes = format!(
            "{{\"schema_version\":6,\"generator_version\":\"0.1.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":null,\"unexpected\":true}}"
        );
    ...
    fn explicit_compatibility_range_can_accept_a_newer_envelope() {
        let bytes = format!(
            "{{\"schema_version\":7,\"generator_version\":\"0.2.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":42}}"
        );
        let compatibility = SchemaCompatibility::inclusive(6, 7).expect("valid range");
    ...
    fn bound_decode_rejects_envelope_payload_identity_mismatch() {
        let bytes = format!(
            "{{\"schema_version\":6,\"generator_version\":\"0.1.0\",\"binary_sha256\":\"{DIGEST_A}\",\"payload\":{{\"binary_id\":\"{DIGEST_B}\",\"value\":7}}}}"
        );
        let error =
