...
fn six_exact_little_endian_vectors_match_the_frozen_mask_and_text() {
    ...
    for (word, exact_bytes, pattern, expected_text) in vectors {
        ...
        let DecodeOutcome::Decoded(instruction) =
            decoder(PACKED_SUB_V1).decode_one(&exact_bytes, START)
        else {
...
#[test]
fn comparison_saturation_function_and_hidden_field_near_misses_remain_typed_mmi_unsupported() {
    for word in [
        0x70a6_3888,
        0x70a6_38c8,
        0x70a6_3c48,
...
#[test]
fn all_four_prior_profiles_remain_immutable_and_typed_unsupported() {
    for profile in [CORE_V1, WORD_SHIFT_V1, PACKED_LOGICAL_V1, PACKED_ADD_V1] {
        for word in [
            0x70a6_3848,
            0x70a6_3948,
            0x70a6_3a48,
...
#[test]
fn every_nested_mmi_selector_differs_from_the_parent_only_for_the_three_subtractions() {
    for function in [0x08_u32, 0x09, 0x28, 0x29] {
        for secondary in 0..32_u32 {
            let word = nested_mmi_word(function, secondary, 5, 6, 7);
            if function == 0x08 && matches!(secondary, 0x01 | 0x05 | 0x09) {
                ...
                let DecodeOutcome::Decoded(instruction) = decode_word(PACKED_SUB_V1, word, START)
                else {
                    panic!("expected 0x{word:08x} to decode");
                };
                assert_eq!(instruction.text, expected_text);
...
#[test]
fn framing_little_endian_address_trailing_and_swapped_cop2_inheritance_are_exact() {
    let word = 0x70a6_3848_u32;
    let bytes = word.to_le_bytes();
    for available in 0..4 {
        assert_eq!(
            decoder(PACKED_SUB_V1).decode_one(&bytes[..available], u64::MAX),
            DecodeOutcome::Truncated { available }
        );
    }

    assert_eq!(decode_word(PACKED_SUB_V1, word, 2), DecodeOutcome::Invalid);
    assert_eq!(
        decode_word(PACKED_SUB_V1, word, u64::from(u32::MAX) + 1),
        DecodeOutcome::Invalid
    );
    let DecodeOutcome::Decoded(last_aligned) =
        decode_word(PACKED_SUB_V1, word, u64::from(u32::MAX) - 3)
    else {
    ...
    assert_eq!(
        decoder(PACKED_SUB_V1).decode_one(&with_trailing, START),
        decoder(PACKED_SUB_V1).decode_one(&bytes, START)
    );
    ...
    for swapped_word in [
        ...
        let swapped_outcome = decoder(PACKED_SUB_V1).decode_one(&swapped_word.to_be_bytes(), START);
        assert_eq!(
            swapped_outcome,
            decode_word(PACKED_SUB_V1, swapped_word.swap_bytes(), START)
        );
        assert_eq!(
            swapped_outcome,
            DecodeOutcome::Unsupported {
...
#[test]
fn linear_sweep_preserves_prior_prefix_adds_subtraction_then_stops_on_residual_mmi() {
    let logical = 0x70a6_3c89_u32;
    let added = 0x70a6_3808_u32;
    let subtracted = 0x70a6_3848_u32;
    let unsupported = 0x70a6_3888_u32;
    let fifth = 0x24a6_0001_u32;
    let bytes = [
        logical.to_le_bytes(),
        added.to_le_bytes(),
    ...
    let mut decoder = decoder(PACKED_SUB_V1);
...
fn exact_profile_identity_is_authoritative_over_the_broad_family() {
    let decoder = decoder(PACKED_SUB_V1);
    assert_eq!(decoder.profile(), PACKED_SUB_V1);
