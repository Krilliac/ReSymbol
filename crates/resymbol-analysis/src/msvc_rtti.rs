...
pub(crate) fn parse_msvc_rtti(
    ...
    'sections: for (section, section_scan_bytes) in scan_plan {
        ...
        let Some(section_end) = section_start.checked_add(section.layout().file_backed_size) else {
...
fn build_scan_plan(
    ...
    for section in eligible {
        ...
        let prefix = u64::from(section.layout().file_backed_size).min(remaining);
        remaining -= prefix;
...
fn is_scan_section(section: &PeSection) -> bool {
    is_type_descriptor_section(section)
        ...
        && section.layout().file_backed_size >= POINTER_SIZE
}
...
fn section_range_is_backed(
    ...
    sections.iter().any(|section| {
        ...
        let Some(backed_end) = start.checked_add(u64::from(section.layout().file_backed_size))
        else {
            return false;
        };
