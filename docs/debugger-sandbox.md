...
agent, live process attach, live breakpoint backend, register access, provider-integrated live memory
...
corroborates the executable available through the process's current path without rehashing the full
file on every read, but it does not make loaded-image bytes immutable against an already-authorized
same-account actor able to duplicate/tamper with handles or mutate process state.
...
- The live-access adapter is movable to one helper worker but intentionally not shared; its write
  method requires exclusive Rust ownership in addition to the future provider's stopped-state gate.
...
2. A queued static edit is an exact-RVA, exact-byte draft. **Create New Patched Binary** requires a
   NOP draft to decode as exactly one complete valid x86-64 instruction, then a worker-owned checked
   plan revalidates the full source identity and expected bytes before no-clobber publication. For
   canonical unprefixed short and near Jcc encodings, the menu can also queue an invert-condition or
