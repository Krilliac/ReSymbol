...
The current alpha can execute eligible external-process analysis plugins. Before first execution,
review the plugin and run `resymbol plugin trust <id>` to approve its exact directory fingerprint.
Any change to the manifest, entrypoint, libraries, or data invalidates that approval. Trust only
process plugins you would run directly: process separation protects ReSymbol from a crash, but is
not an operating-system sandbox and does not restrict the child's ambient user access.
